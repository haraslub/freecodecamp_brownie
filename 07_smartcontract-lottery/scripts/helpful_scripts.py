...
def get_contract(contract_name):
    ...
    if defined, otherwrise, it will deploy a mock version of that contract,
    ...
    if network.show_active() in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        if len(contract_type) <= 0:
            deploy_mocks()
        contract = contract_type[-1]    # the most recent contract deployed
    else:
...
def fund_with_link(contract_address, account=None, link_token=None, amount=100000000000000000): # = 0.1 LINK
    ...
    # or the second way (via interfaces):
